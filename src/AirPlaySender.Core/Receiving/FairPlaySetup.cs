namespace AirPlaySender.Core.Receiving;

/// <summary>
/// The <c>/fp-setup</c> step of AirPlay Mirroring — "FairPlay" being Apple's
/// actual DRM/content-protection system, not a device-authentication scheme
/// like the rest of this project's pairing work. This class is fundamentally
/// different from everything else in this codebase and deliberately kept
/// separate and clearly labeled, rather than blended in stylistically with
/// the from-scratch protocol work elsewhere:
///
/// The four 142-byte reply blobs below are NOT derived, computed, or
/// reverse-engineered by this project. They are exact copies of responses
/// captured from a real Apple FairPlay exchange, reproduced here from
/// UxPlay's <c>lib/fairplay_playfair.c</c> (GPLv3) — which itself states
/// they come from "OmgHax", the original disassembly-based reverse
/// engineering of Apple's FairPlay client. Nobody, in any open-source
/// project, has published the actual FairPlay algorithm; every working
/// AirPlay Mirroring receiver either replays these same captured bytes (as
/// here) or reimplements Apple's cipher from disassembly for the *next*
/// step (decrypting the per-stream key during SETUP) — a materially
/// different, and more legally exposed, kind of reverse engineering than
/// this project's own pairing/audio work. See the Phase 2 discussion in
/// this project's history for the explicit decision to proceed anyway, on
/// the basis that this is a private, non-distributed personal build.
///
/// Wire shape (confirmed against UxPlay's <c>raop_handler_fpsetup</c> +
/// <c>fairplay_playfair.c</c>, not guessed):
///  - round 1: client sends 16 bytes; byte[14] selects one of 4 canned
///    142-byte replies (byte[4] must be the FairPlay protocol version, 3).
///  - round 2: client sends 164 bytes (byte[4] must again be 3); we save
///    the whole thing (needed much later, during SETUP, to decrypt the
///    actual per-stream key — the one piece of this that isn't just replay)
///    and reply with a fixed 12-byte header followed by the request's own
///    last 20 bytes echoed back.
/// </summary>
public sealed class FairPlaySetupSession
{
    private static readonly byte[][] ReplyMessages =
    [
        [],
        [],
        [],
        [],
    ];

    private static readonly byte[] FpHeader = [0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14];

    /// <summary>The 164-byte round-2 request, saved for the later SETUP step that decrypts the real per-stream key. Not yet used — that step isn't implemented.</summary>
    public byte[]? KeyMessage { get; private set; }

    public byte[]? Handle(byte[] request) => request.Length switch
    {
        16 => HandleRound1(request),
        164 => HandleRound2(request),
        _ => null,
    };

    private static byte[]? HandleRound1(byte[] request)
    {
        if (request[4] != 0x03) return null; // unsupported FairPlay protocol version
        int mode = request[14];
        if (mode is < 0 or > 3) return null;
        return (byte[])ReplyMessages[mode].Clone();
    }

    private byte[]? HandleRound2(byte[] request)
    {
        if (request[4] != 0x03) return null;
        // byte[12] becomes the "mode" FairPlayCipher.Decrypt indexes
        // message_key/message_iv with, much later during SETUP — those
        // tables only have 4 entries (found by code review: nothing
        // validated this, so a malformed round-2 body could crash SETUP
        // with an unhandled IndexOutOfRangeException instead of a clean
        // 400 here, where the request is still in hand to reject outright).
        int mode = request[12];
        if (mode is < 0 or > 3) return null;
        KeyMessage = (byte[])request.Clone();
        return [.. FpHeader, .. request[144..164]];
    }
}
