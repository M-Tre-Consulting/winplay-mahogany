...
/// <summary>
/// Just enough of an RTSP server to drive <see cref="HapPairVerifyAccessorySession"/>
/// over a real loopback TCP socket against this project's own (unmodified)
/// controller-side <c>Pairing.PairVerifyClient</c> — the same "run the real
/// counterpart against a minimal fake" trick as <see cref="FakeAirPlay2Receiver"/>,
/// just for the accessory-role HAP pair-verify instead of transient pair-setup.
/// Handles exactly one connection, exactly two <c>POST /pair-verify</c>
/// round-trips (M1→M2, M3→M4) — nothing else.
/// </summary>
internal sealed class FakeHapAccessory : IAsyncDisposable
{
    private readonly TcpListener _listener;
    private readonly HapPairVerifyAccessorySession _session;
    private TcpClient? _client;
    private Task? _serveTask;

    public int Port { get; }

    public FakeHapAccessory(HapPairVerifyAccessorySession session)
    {
        _session = session;
    ...
    private async Task ServeAsync()
    {
        ...
        try
        {
            for (int i = 0; i < 2; i++) // exactly M1 and M3
            {
                ...
                if (respBody is null) throw new InvalidOperationException("HapPairVerifyAccessorySession rejected the request");
